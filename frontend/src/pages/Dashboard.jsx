...
import { 
  LayoutDashboard, History, MapPin, Car, CreditCard, User as UserIcon, 
  Settings, LogOut, Navigation2, Zap, Leaf, CheckCircle2, ChevronRight, PlusCircle, SlidersHorizontal,
...
const TokenRefillPaymentModal = ({ user, onClose, onSuccess }) => {
  ...
  const walletBalance = user && user.walletBalance !== undefined ? user.walletBalance : 1000;
  const plannerCredits = user && user.plannerCredits !== undefined ? user.plannerCredits : 10;
  ...
  const handlePay = async () => {
    ...
    if (payMethod === 'card') {
      if (!cardNum || cardNum.replace(/\s/g,'').length < 16) return setError('Enter a valid 16-digit card number.');
      if (!cardName) return setError('Enter the cardholder name.');
      if (!cardExpiry) return setError('Enter an expiry date (MM/YY).');
      if (!cardCvv || cardCvv.length < 3) return setError('Enter a valid 3-digit CVV.');
    }
    if (payMethod === 'upi') {
      if (!mockUpi || !mockUpi.includes('@')) return setError('Enter a valid UPI ID (e.g. driver@oksbi).');
    }
    if (payMethod === 'wallet' && isWalletInsufficient) {
      return setError(`Insufficient wallet balance. Refill at least ₹${selectedBundle.cost} first.`);
    }
  ...
  return (
    <div className="token-modal-overlay fixed inset-0 z-[200] flex items-center justify-center p-4"
      ...
      <div className="token-modal-panel relative w-full max-w-xl bg-[#111318] border border-cyber-gray-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-cyber-gray-800 bg-gradient-to-r from-cyber-green/5 to-transparent">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h2 className="text-sm font-extrabold text-white tracking-wide">Premium Token Bundles</h2>
              <p className="text-[10px] text-gray-500 font-mono tracking-widest">ONWHEEL EV INTELLIGENCE COCKPIT</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-cyber-gray-800 text-gray-500 hover:text-white transition">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {step === 'select' && (
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between bg-[#0b0c10] border border-cyber-gray-900 rounded-xl px-4 py-3 text-xs">
              <span className="text-gray-500 font-mono tracking-wider">WALLET BALANCE</span>
              <span className="text-cyber-green font-extrabold font-mono">₹{walletBalance.toLocaleString()}</span>
              <span className="text-gray-500 font-mono tracking-wider">CURRENT TOKENS</span>
              <span className="text-cyber-accent font-extrabold font-mono">⚡ {plannerCredits}</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {BUNDLES.map(b => (
                ...
                >
                  {b.badge && (
                    <span className={`absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] font-extrabold uppercase tracking-widest px-2 py-0.5 rounded-full bg-cyber-green text-black whitespace-nowrap`}>
                      {b.badge}
                    </span>
                  )}
                  <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${b.highlight}`}>{b.name}</p>
                  <p className="text-lg font-extrabold text-white">⚡ {b.tokens}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">tokens</p>
                  <div className="mt-3 pt-2 border-t border-white/5">
                    <p className="text-sm font-extrabold text-white">₹{b.cost}</p>
                    {b.savings && <p className="text-[9px] text-cyber-green font-bold">Save ₹{b.savings}!</p>}
                  </div>
                </button>
              ))}
            </div>
        ...
        {step === 'checkout' && (
          <div className="p-6 space-y-5">
            ...
            <div className="bg-[#0b0c10] border border-cyber-gray-900 rounded-xl p-4 space-y-2.5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Order Summary</p>
              <div className="flex justify-between text-xs">
                <span className="text-gray-300">{selectedBundle.name}</span>
                <span className="text-white font-bold">⚡ {selectedBundle.tokens} tokens</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-gray-400">Unit rate</span>
                <span className="text-gray-300">₹{(selectedBundle.cost / selectedBundle.tokens).toFixed(1)} / token</span>
              </div>
              {selectedBundle.savings && (
                <div className="flex justify-between text-xs">
                  <span className="text-cyber-green">Bundle Discount</span>
                  <span className="text-cyber-green font-bold">−₹{selectedBundle.savings}</span>
                </div>
              )}
            ...
            {/* Payment method tabs */}
            <div>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Payment Method</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { id: 'wallet', label: 'CyberPass Wallet', sub: `₹${walletBalance.toLocaleString()} avail.` },
                  { id: 'card',   label: 'Debit / Credit', sub: 'Simulated checkout' },
            ...
            {payMethod === 'wallet' && isWalletInsufficient && (
              <div className="bg-red-500/10 border border-red-500/25 rounded-xl p-3 text-xs text-red-400">
                ⚠ Insufficient wallet balance for this bundle. Add ₹{selectedBundle.cost - walletBalance} more or choose another payment method.
              </div>
            )}
            ...
            {payMethod === 'card' && (
              <div className="space-y-3">
                <input
                  type="text" maxLength={19} placeholder="Card Number (16 digits)"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  />
                  <input
                    type="password" maxLength={4} placeholder="CVV"
                ...
                <p className="text-[9px] text-gray-600 flex items-center space-x-1">
                  <Lock className="w-2.5 h-2.5" /><span>Simulated secure card — no real charges</span>
                </p>
              </div>
            )}
            ...
            {payMethod === 'upi' && (
              <div className="space-y-2">
                <input
                  type="text" placeholder="Your UPI ID (e.g. driver@oksbi)"
                  value={mockUpi} onChange={e => setMockUpi(e.target.value)}
                ...
                />
                <p className="text-[9px] text-gray-600 flex items-center space-x-1">
                  <Lock className="w-2.5 h-2.5" /><span>Mock UPI — simulated for testing only</span>
                </p>
              </div>
            )}
        ...
        {step === 'loading' && (
          <div className="p-10 flex flex-col items-center space-y-6 text-center">
            ...
            <div className="relative w-20 h-20 flex items-center justify-center">
              ...
              <div className="absolute inset-2 rounded-full border border-cyber-green/40" style={{ animation: 'spinGlow 1.4s linear infinite reverse' }} />
              <Zap className="w-8 h-8 text-cyber-green fill-cyber-green" style={{ filter: 'drop-shadow(0 0 8px #00E576)' }} />
            </div>
        ...
        {step === 'success' && (
          <div className="p-10 flex flex-col items-center space-y-5 text-center">
            <div className="token-success-pop w-20 h-20 rounded-full bg-cyber-green/15 border-2 border-cyber-green flex items-center justify-center"
              style={{ boxShadow: '0 0 30px rgba(0,229,118,0.25)' }}>
              <CheckCircle2 className="w-10 h-10 text-cyber-green" />
            </div>
            <div className="space-y-1">
              <h3 className="text-lg font-extrabold text-white">Tokens Unlocked! 🎉</h3>
              <p className="text-sm text-cyber-green font-mono">+{selectedBundle.tokens} Premium Intelligence Tokens</p>
              <p className="text-xs text-gray-500">Your AI cockpit is now supercharged and ready.</p>
            </div>
            <div className="w-full bg-[#0b0c10] border border-cyber-gray-900 rounded-xl p-4 space-y-1.5 text-xs text-left">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-cyber-green font-bold">+{selectedBundle.tokens} ⚡</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Amount charged</span>
                <span className="text-white font-bold">{payMethod === 'wallet' ? `₹${selectedBundle.cost} from Wallet` : `₹${selectedBundle.cost} via ${payMethod.toUpperCase()}`}</span>
              </div>
            </div>
            <button onClick={onClose} className="w-full btn-cyber-primary py-2.5 text-sm font-bold tracking-wider uppercase">
              Back to Dashboard
            </button>
          </div>
        )}
        ...
        {(step === 'select' || step === 'checkout') && (
          <div className="px-6 pb-4 flex items-center justify-center space-x-2 text-[9px] text-gray-600 font-mono">
            ...
            <span>256-bit AES encrypted · Secured by OnWheel EV Cyber Ledger</span>
          </div>
        )}
      </div>
    </div>
...
const Dashboard = ({ user, onLogout, onProfileUpdate }) => {
  const [selectedModel, setSelectedModel] = useState(user ? user.vehicleModel : 'Tata Nexon EV Max');
  const [selectedCapacity, setSelectedCapacity] = useState(user ? user.batteryCapacity : 40.5);
  ...
  const handleAddLocation = async (e) => {
    ...
    try {
      ...
      const currentLocs = user && user.savedLocations ? [...user.savedLocations] : [];
      const updatedUser = {
        ...user,
  ...
  const handleDeleteLocation = async (locId) => {
    ...
    try {
      ...
      const currentLocs = user && user.savedLocations ? user.savedLocations.filter(loc => loc._id !== locId) : [];
      const updatedUser = {
        ...user,
  ...
  const handleRefillWallet = async (e) => {
    ...
    try {
      ...
      const currentTrans = user && user.transactions ? [...user.transactions] : [
        { _id: 'tx1', amount: 500, description: 'Signup Promotional EV Credits Credited', date: new Date() },
        { _id: 'tx2', amount: -120, description: 'Initial DLF Cyber Charge Billing', date: new Date() }
      ];
  ...
  const handleBuyPlannerTokens = async () => {
    ...
    try {
      ...
      const amountVal = 100;
      const currentCredits = user && user.plannerCredits !== undefined ? user.plannerCredits : 10;
      const currentTrans = user && user.transactions ? [...user.transactions] : [];
      
      const newTx = {
        _id: 'tx-local-' + Date.now(),
        amount: -amountVal,
        description: 'Purchased 10 Premium Route Planner Tokens (Local Simulation)',
        date: new Date()
      };
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      ...
      console.warn('ImageKit credentials missing or network offline. Utilizing premium high-fidelity local screenshot FileReader fallback...');
  ...
  const myVehicles = user && user.vehicles && user.vehicles.length > 0
    ? user.vehicles
    : [
        {
  ...
  const handleAddVehicle = async (e) => {
    ...
    try {
      ...
      const mockVehicle = {
        ...
        model: newModel,
        capacity: parseFloat(newCapacity),
        range: newRange ? parseFloat(newRange) : Math.round(parseFloat(newCapacity) * 9.5)
      };
      
      const currentVehicles = user.vehicles && user.vehicles.length > 0 
        ? [...user.vehicles] 
        : [{ _id: 'default-active-ev', model: user.vehicleModel, capacity: user.batteryCapacity, range: Math.round(user.batteryCapacity * 9.5) }];
      ...
      const updatedVehicles = myVehicles.map(v => {
        if (v._id === editingVehicleId) {
          return {
            ...
            model: editModel,
            capacity: parseFloat(editCapacity),
            range: editRange ? parseFloat(editRange) : Math.round(parseFloat(editCapacity) * 9.5)
          };
        }
        return v;
      });
      const updatedTarget = updatedVehicles.find(v => v._id === editingVehicleId);
      const wasActive = user.vehicleModel === myVehicles.find(v => v._id === editingVehicleId)?.model;
      const updatedUser = {
        ...user,
        vehicles: updatedVehicles,
        ...(wasActive ? {
    ...
    try {
      ...
      if (historyRes.data.success) {
        ...
        if (userTrips && userTrips.length > 0) {
          ...
          const totalDistance = userTrips.reduce((acc, t) => acc + t.totalDistance, 0);
          setMetrics({
      ...
      if (localTrips.length > 0) {
        ...
        setRecentTrips(formatted.slice(0, 4));
        const totalDistance = localTrips.reduce((acc, t) => acc + t.totalDistance, 0);
        setMetrics({
        ...
        <TokenRefillPaymentModal
          ...
          onSuccess={(updatedUser, bundle) => {
            if (updatedUser && onProfileUpdate) {
              ...
              const current = user || {};
              const newCredits = (current.plannerCredits !== undefined ? current.plannerCredits : 10) + bundle.tokens;
        ...
        >
            ...
            <>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  ...
                  <p className="text-gray-400 text-sm mt-0.5">Here's your electric mobility trip summary.</p>
                </div>

                <Link to="/planner" className="btn-cyber-primary text-xs font-bold py-2 px-4">
                  Plan New Route
                </Link>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-[#121212] border border-cyber-gray-800 rounded-xl p-4.5 text-left shadow-md">
                  <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Total Trips</span>
                  <span className="text-2xl font-extrabold text-white mt-1.5 block">{metrics.totalTrips}</span>
                </div>

                <div className="bg-[#121212] border border-cyber-gray-800 rounded-xl p-4.5 text-left shadow-md">
                  <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Distance Covered</span>
                  <span className="text-2xl font-extrabold text-cyber-green mt-1.5 block">{metrics.distanceCovered.toLocaleString()} km</span>
                </div>
                ...
                <div className="bg-[#121212] border border-cyber-gray-800 rounded-xl p-4.5 text-left shadow-md">
                  ...
                  <span className="text-2xl font-extrabold text-cyber-accent mt-1.5 block">{metrics.carbonSaved} kg</span>
                </div>

                <div className="bg-[#121212] border border-cyber-gray-800 rounded-xl p-4.5 text-left shadow-md">
                  <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Total Cost Saved</span>
                  <span className="text-2xl font-extrabold text-white mt-1.5 block">₹{metrics.totalSpent.toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                ...
                <div className="md:col-span-7 bg-[#121212] border border-cyber-gray-800 rounded-2xl p-5 shadow-lg space-y-4">
                  <h3 className="font-bold text-white text-base border-b border-cyber-gray-900 pb-3">Recent Trips</h3>
                  
                  <div className="space-y-3">
                    {recentTrips.length > 0 ? (
                      recentTrips.map((trip) => (
                        <div key={trip.id} className="flex justify-between items-center bg-[#0b0c10] border border-cyber-gray-900 hover:border-cyber-green/20 p-3.5 rounded-xl transition-all">
                          <div className="text-left">
                            ...
                            <span className="text-[10px] text-gray-500 mt-1 block">{trip.date} • {trip.stops} Stops</span>
                          </div>

                          <div className="text-right">
                            <span className="text-xs font-bold text-white block">{trip.dist} km</span>
                      ...
                      <div className="p-8 text-center bg-[#0b0c10]/40 border border-cyber-gray-950 rounded-xl">
                        <p className="text-xs text-gray-500 font-semibold">No recent trips logged.</p>
                        <Link to="/planner" className="text-[10px] text-cyber-green hover:underline font-bold mt-2 inline-block">
                ...
                <div className="md:col-span-5 bg-[#121212] border border-cyber-gray-800 rounded-2xl p-5 shadow-lg space-y-5 text-left">
                  <h3 className="font-bold text-white text-base border-b border-cyber-gray-900 pb-3">My Vehicle</h3>
                  ...
                  <div className="bg-[#0b0c10] border border-cyber-gray-900 p-4 rounded-xl flex items-center space-x-3.5">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-500 uppercase font-semibold">Active EV Model</span>
                      <span className="text-sm font-extrabold text-white mt-0.5">{vehicleName}</span>
                      <span className="text-[10px] text-cyber-accent mt-0.5 font-medium">Battery Pack: {batteryCap} kWh</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#0b0c10] border border-cyber-gray-900 p-4 rounded-xl space-y-3.5">
                    <div className="flex justify-between items-center text-xs font-semibold">
                      <span className="text-gray-400">Current Charge level</span>
                      <span className="text-cyber-green font-extrabold">60%</span>
                    </div>
                    
                    <div className="h-2.5 bg-cyber-gray-900 rounded-full overflow-hidden w-full border border-cyber-gray-800">
                      <div className="h-full bg-cyber-green rounded-full shadow-cyber-glow" style={{ width: '60%' }} />
                    </div>
          ...
          {activeTab === 'locations' && (
            <div className="space-y-6 animate-fade-in text-left">
              <h3 className="font-bold text-white text-lg border-b border-cyber-gray-800 pb-3">Saved Locations</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {user && user.savedLocations && user.savedLocations.length > 0 ? (
                  user.savedLocations.map((loc) => (
                    ...
                    >
                      <div>
                        ...
                        <span className="text-[9px] font-mono text-gray-600 block mt-0.5">GPS: {loc.lat?.toFixed(4)}, {loc.lng?.toFixed(4)}</span>
                      </div>
                  ...
                  <div className="md:col-span-2 p-8 text-center bg-[#0b0c10]/40 border border-cyber-gray-950 rounded-xl">
                    <p className="text-xs text-gray-500 font-semibold">No saved locations found on your profile.</p>
                  </div>
                )}
              </div>

              <form onSubmit={handleAddLocation} className="bg-[#121212] border border-cyber-gray-800 p-5 rounded-2xl space-y-4 max-w-md">
                <h4 className="font-bold text-white text-sm">Add New Saved Location</h4>
                ...
                >
                  {locSaving ? 'Saving coordinates...' : 'Save Location Coordinates'}
                </button>
              </form>
            </div>
          )}
          ...
          {activeTab === 'vehicles' && (
            <div className="space-y-8 animate-fade-in text-left">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-cyber-gray-800 pb-4">
                <div>
                  <h3 className="font-bold text-white text-xl">My Fleet Garage</h3>
                  <p className="text-gray-400 text-xs mt-0.5">Tweak telemetries, add custom battery capacities, and switch active rides.</p>
                </div>
              ...
              {isAdding && (
                ...
                >
                  ...
                  <form onSubmit={handleAddVehicle} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end text-xs text-left">
                    <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                      <label className="font-bold uppercase tracking-wider">EV Model Name</label>
                      <input
                        type="text"
                        required
                    ...
                    <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                      <label className="font-bold uppercase tracking-wider">Battery Capacity (kWh)</label>
                      <input
                        type="number"
                        step="0.1"
                        required
                    ...
                    <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                      <label className="font-bold uppercase tracking-wider">Estimated Range (km)</label>
                      <input
                        type="number"
                  ...
                  return (
                    ...
                    >
                      ...
                      {isEditingThis ? (
                        ...
                        <form onSubmit={handleEditVehicleSubmit} className="space-y-4 text-xs text-left">
                          ...
                          <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                            <label className="font-bold uppercase tracking-wider">EV Model</label>
                            <input
                              type="text"
                              required
                              value={editModel}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                              <label className="font-bold uppercase tracking-wider">Capacity (kWh)</label>
                              <input
                                type="number"
                                step="0.1"
                                required
                            ...
                            <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                              <label className="font-bold uppercase tracking-wider">Range (km)</label>
                              <input
                                type="number"
                                required
                                value={editRange}
                        ...
                        <div className="space-y-4">
                          <div className="flex items-start space-x-3">
                            <Car className={`w-8 h-8 ${isCurrentlyActive ? 'text-cyber-green' : 'text-gray-400'}`} />
                            <div>
                              <h4 className="font-bold text-white text-base leading-tight">{vehicle.model}</h4>
                              <p className="text-[10px] text-gray-500 mt-0.5">Saved Configuration</p>
                            </div>
                          </div>
                          ...
                          <div className="text-xs space-y-2 border-t border-cyber-gray-900 pt-3 text-gray-400">
                            <p className="flex justify-between">
                              ...
                              <b className="text-white">{vehicle.capacity} kWh</b>
                            </p>
                            <p className="flex justify-between">
                              <span>Range Estimate:</span>
                              <b className="text-cyber-accent">{vehicle.range || Math.round(vehicle.capacity * 9.5)} km</b>
                            </p>
                            <p className="flex justify-between">
                              <span>Charging Standard:</span>
                              <b className="text-gray-400 font-mono text-[10px]">CCS Type 2</b>
                            </p>
                          </div>
                            ...
                            <div className="flex space-x-1.5">
                              <button
                                onClick={() => {
                                  ...
                                  setEditRange(vehicle.range || Math.round(vehicle.capacity * 9.5));
                                  setIsAdding(false);
                                }}
                              ...
                              <button
                                ...
                                disabled={isCurrentlyActive || myVehicles.length <= 1}
          ...
          {activeTab === 'payments' && (
            <div className="space-y-8 animate-fade-in text-left">
              <h3 className="font-bold text-white text-lg border-b border-cyber-gray-800 pb-3">Payments & CyberPass Billing</h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="bg-gradient-to-br from-[#121212] to-[#08090C] border border-cyber-green/50 p-5 rounded-2xl h-44 flex flex-col justify-between shadow-cyber-glow relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[8px] font-mono tracking-widest text-cyber-green border border-cyber-green px-1.5 py-0.5 rounded">CYBER_PASS</span>
                    </div>
                    <div className="text-left font-mono">
                      <p className="text-[10px] text-gray-500 tracking-wider">CREDIT BALANCE</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">₹{user && user.walletBalance !== undefined ? user.walletBalance.toLocaleString() : '1,000'}.00</h3>
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-[#121212] to-[#08090C] border border-cyber-accent/50 p-5 rounded-2xl h-44 flex flex-col justify-between shadow-cyber-glow relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[8px] font-mono tracking-widest text-cyber-accent border border-cyber-accent px-1.5 py-0.5 rounded">PREMIUM_AI_COCKPIT</span>
                    </div>
                    <div className="text-left font-mono">
                      <p className="text-[10px] text-gray-500 tracking-wider">PREMIUM PLANNER TOKENS</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">⚡ {user && user.plannerCredits !== undefined ? user.plannerCredits : 10} Tokens</h3>
                    </div>
                  ...
                  <div className="bg-cyber-card border border-cyber-gray-800 p-5 rounded-2xl space-y-5">
                    <h4 className="font-bold text-white text-xs uppercase tracking-wider">Refill Pass Wallet</h4>
                    ...
                    {refillMode === 'instant' ? (
                      ...
                      <form onSubmit={handleRefillWallet} className="space-y-4">
                        <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                          <label className="font-bold uppercase tracking-wider">Refill Amount (₹)</label>
                          <input
                            type="number"
                            required
                            min="50"
                        ...
                        >
                          {isRefilling ? 'Processing CyberCard...' : 'Refill Instantly'}
                        </button>
                      </form>
                    ) : (
                      /* Manual Payments Upload verified screenshot flow */
                      <form onSubmit={handleManualPaymentSubmit} className="space-y-4 text-xs text-left">
                        {/* Submethod Selection */}
                        ...
                        {manualMethod === 'upi' && (
                          <div className="flex flex-col space-y-1 text-[10px] text-gray-500">
                            <label className="font-bold uppercase tracking-wider">Your UPI ID (Sender)</label>
                            <input
                              type="text"
                              required
                        ...
                        <div className="flex flex-col space-y-1 text-[10px] text-gray-500">
                          <label className="font-bold uppercase tracking-wider">12-Digit Transaction UTR Number</label>
                          <input
                            type="text"
                            required
                            maxLength={12}
                          ...
                          />
                          <span className="text-[8px] text-gray-500 block">Unique UTR is strictly validated by administrator.</span>
                        </div>

                        {/* File Upload Box ImageKit */}
                        <div className="flex flex-col space-y-1.5 text-[10px] text-gray-500">
                          <label className="font-bold uppercase tracking-wider">Upload Payment Screenshot Receipt</label>
                          <div className={`border-2 border-dashed rounded-xl p-3 flex flex-col items-center justify-center relative cursor-pointer hover:bg-[#0b0c10]/40 transition ${
                            uploadSuccess 
                              ? 'border-cyber-green' 
                              : uploadLoading 
                            ...
                            {uploadLoading ? (
                              <div className="text-center py-2">
                                ...
                                <span className="text-cyber-accent text-[9px] font-mono tracking-widest uppercase">Uploading to ImageKit.io...</span>
                              </div>
                            ) : uploadSuccess ? (
                              ...
                              <div className="text-center py-2">
                                <p className="text-gray-400 font-semibold text-[9px] uppercase tracking-wider">Choose receipt screenshot image</p>
                                <span className="text-[8px] text-gray-500 block mt-0.5">Direct ImageKit.io Upload enabled</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <button
                          type="submit"
                          disabled={manualSaving || uploadLoading || !uploadSuccess}
                        ...
                        >
                          {manualSaving ? 'Submitting to Admin...' : 'Submit Payment Verification'}
                        </button>
                      </form>
                    )}
                  </div>
                ...
                <div className="lg:col-span-7 bg-cyber-card border border-cyber-gray-800 rounded-2xl p-5 space-y-4 shadow-lg">
                  <h4 className="font-bold text-white text-sm border-b border-cyber-gray-900 pb-3 flex justify-between items-center">
                    ...
                    <span className="text-[9px] text-gray-500 font-normal">Auto-settled via Grid Smart Contracts</span>
                  </h4>
                  ...
                  <div className="space-y-3.5 max-h-80 overflow-y-auto pr-1">
                    {user && user.transactions && user.transactions.length > 0 ? (
                          ...
                          >
                            <div className="text-left space-y-1">
                              ...
                              <span className="text-[9px] text-gray-500 block">
                                {new Date(tx.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} at {new Date(tx.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </span>
                            </div>
                            
                            <div className="text-right">
                              <span className={`text-xs font-extrabold font-mono ${isRefill ? 'text-cyber-green' : 'text-red-400'}`}>
                      ...
                      <div className="p-8 text-center bg-[#0b0c10]/40 border border-cyber-gray-950 rounded-xl">
                        <p className="text-xs text-gray-500 font-semibold">No recent transactions recorded.</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-[#121212] border border-cyber-gray-800 p-5 rounded-2xl space-y-4">
                <h4 className="font-bold text-white text-sm border-b border-cyber-gray-900 pb-3 flex justify-between items-center text-left">
                  ...
                  <span className="text-[8px] font-mono text-cyber-accent">ImageKit.io Screens & UTR12 Unique Ledger</span>
                </h4>
                  ...
                  {myRequests.length > 0 ? (
                    myRequests.map((req) => (
                      ...
                      >
                        <div className="space-y-1">
                          <div className="flex items-center space-x-2">
                            <span className="text-xs font-extrabold text-white">₹{req.amount.toLocaleString()}</span>
                            <span className="text-[9px] text-gray-500 font-mono tracking-wider">({req.paymentMethod.toUpperCase()})</span>
                          </div>
                          <p className="text-[10px] text-gray-400 font-mono">UTR: <b className="text-gray-200 tracking-wider">{req.utr}</b></p>
                          <span className="text-[8px] text-gray-600 block">{new Date(req.createdAt).toLocaleDateString()}</span>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          >
                            <img src={req.screenshotUrl} alt="Manual Refill Screen" className="object-cover w-full h-full opacity-70 hover:opacity-100 transition" />
                          </a>

                          <span className={`px-2 py-1 text-[9px] font-bold uppercase rounded font-mono ${
                            req.status === 'pending'
                    ...
                    <div className="md:col-span-2 p-8 text-center bg-[#0b0c10]/40 border border-cyber-gray-950 rounded-xl">
                      <p className="text-xs text-gray-500 font-semibold">Zero manual payment verification requests logged.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6 animate-fade-in text-left">
              <h3 className="font-bold text-white text-lg border-b border-cyber-gray-800 pb-3">Edit Driver Profile</h3>
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="flex flex-col space-y-1.5 text-xs text-gray-400">
                  ...
                  <input type="text" defaultValue={userName} className="w-full bg-[#0b0c10] border border-cyber-gray-900 focus:border-cyber-green rounded-lg py-2.5 px-3 text-white outline-none transition" />
                </div>
                <div className="flex flex-col space-y-1.5 text-xs text-gray-400">
                  <label className="font-bold uppercase tracking-wider">Email Address</label>
                  <input type="email" defaultValue={user ? user.email : 'driver@onwheel.ev'} className="w-full bg-[#0b0c10] border border-cyber-gray-900 focus:border-cyber-green rounded-lg py-2.5 px-3 text-white outline-none transition" />
                </div>
                <button type="submit" className="w-full btn-cyber-primary py-2.5 text-xs font-bold">Save Profile Modifications</button>
              </form>
            </div>
          )}
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6 animate-fade-in text-left">
              <h3 className="font-bold text-white text-lg border-b border-cyber-gray-800 pb-3">Platform Preferences</h3>
              ...
              <div className="bg-[#121212] border border-cyber-gray-800 rounded-2xl p-5 space-y-5 max-w-md text-xs text-gray-400">
                <div className="flex justify-between items-center border-b border-cyber-gray-900 pb-3">
                  <div>
                    <h4 className="font-bold text-white text-sm">Measuring Distance Units</h4>
                    <p className="text-[10px] text-gray-500 mt-0.5">Toggle kilometers vs miles scales.</p>
                  </div>
                ...
                <div className="flex justify-between items-center border-b border-cyber-gray-900 pb-3">
                  <div>
                    <h4 className="font-bold text-white text-sm">Auto Battery Warning Alerts</h4>
                    <p className="text-[10px] text-gray-500 mt-0.5">Alert coordinates when battery falls low.</p>
                  </div>
                  <input type="checkbox" defaultChecked className="accent-cyber-green cursor-pointer" />
                </div>
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-white text-sm">Active Navigation Algorithm</h4>
                    <p className="text-[10px] text-gray-500 mt-0.5">Choose route charging parameters.</p>
                  </div>
                  <select className="bg-cyber-card border border-cyber-gray-800 text-white rounded p-1">
