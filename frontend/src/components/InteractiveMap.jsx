...
const InteractiveMap = ({ 
  ...
  useEffect(() => {
    if (!mapInstanceRef.current && window.L && mapContainerRef.current) {
      ...
      // CartoDB Dark Matter map layers (Deep Charcoal futuristic styling)
  ...
  const loadChargingStations = async (lat, lng) => {
    ...
    try {
      ...
      console.warn('API Locator offline. Drawing procedural stations nearby...');
    }
    ...
    const localGenerated = [
      { id: 'st-local-1', name: 'ChargeZone DC Grid', lat: lat + 0.004, lng: lng - 0.003, power: 120, status: 'Available', type: 'CCS Type 2' },
      { id: 'st-local-2', name: 'Tata Power EZ Charger', lat: lat - 0.007, lng: lng + 0.005, power: 60, status: 'Available', type: 'DC Fast Port' },
      { id: 'st-local-3', name: 'Statiq Charging Station', lat: lat + 0.008, lng: lng + 0.004, power: 150, status: 'Busy', type: 'CCS2 Fast Charging' },
      { id: 'st-local-4', name: 'Ather Fast Grid', lat: lat - 0.002, lng: lng - 0.008, power: 60, status: 'Available', type: 'AC Type 2' }
    ];
  ...
  useEffect(() => {
    ...
    if (strandedMarkerRef.current) { map.removeLayer(strandedMarkerRef.current); strandedMarkerRef.current = null; }
    if (rescuerMarkerRef.current) { map.removeLayer(rescuerMarkerRef.current); rescuerMarkerRef.current = null; }
    ...
    if (mode === 'browse') {
      ...
      strandedMarkerRef.current = window.L.marker(emergencyCenter, { icon: userIcon })
        .addTo(map)
        .bindPopup('<b class="text-red-400">Emergency Alert</b><br/><span class="text-xs">Stranded with 15% charge. Radar scanning rescue vans...</span>')
        .openPopup();
      
      const localProviders = [
        { id: 'ep-1', name: 'PlugNGo Mobile Assist', lat: 17.452, lng: 78.399, power: 50, type: 'Rescue Charger', status: 'Available' },
        { id: 'ep-2', name: 'ChargeMate On-site Grid', lat: 17.431, lng: 78.365, power: 60, type: 'Rescue Tow Van', status: 'Available' }
      ];
      ...
      localProviders.forEach(p => {
        ...
        const marker = window.L.marker([p.lat, p.lng], { icon: provIcon })
          ...
          .on('click', () => {
            ...
            if (onSelectStation) onSelectStation({ id: p.id, providerName: p.name, rating: 4.9, pricing: '₹25/kWh', type: p.type });
          });
        stationMarkersGroupRef.current.addLayer(marker);
      });
  ...
  const decodeAndDrawOSRMRoute = async () => {
    ...
    let startCoords = [78.4867, 17.3850];
    ...
    try {
      const routeUrl = `https://router.project-osrm.org/route/v1/driving/${startCoords[0]},${startCoords[1]};${endCoords[0]},${endCoords[1]}?overview=full&geometries=geojson`;
      const res = await axios.get(routeUrl);
      
      if (res.data.routes && res.data.routes[0]) {
        const geojson = res.data.routes[0].geometry;
        const coordinates = geojson.coordinates.map(c => [c[1], c[0]]);
        ...
        window.L.marker(coordinates[coordinates.length - 1], { icon: endIcon }).addTo(map);

        const carIcon = createNeonIcon('#00E576', '🚗');
        carMarkerRef.current = window.L.marker(coordinates[0], { icon: carIcon }).addTo(map);
        ...
        if (routeData && routeData.suggestedStops) {
          routeData.suggestedStops.forEach(stop => {
            const stopCoords = stop.lat && stop.lng ? [stop.lat, stop.lng] : coordinates[Math.floor(coordinates.length * 0.4)];
            const stopIcon = createNeonIcon('#00E576', '⚡');
            window.L.marker(stopCoords, { icon: stopIcon })
              .addTo(map)
              .bindPopup(`<b>${stop.name}</b><br/><span class="text-xs">${stop.chargerType || 'Fast Charger'}</span>`);
          });
        }
      }
    } catch (e) {
      console.warn('OSRM routing network timeout, drawing fallback polyline...');
      ...
      const carIcon = createNeonIcon('#00E576', '🚗');
      carMarkerRef.current = window.L.marker(fallbackPoints[0], { icon: carIcon }).addTo(map);
    } finally {
  ...
  useEffect(() => {
    let interval = null;
    if (navPlaying && mode === 'route' && routePointsRef.current.length > 0 && carMarkerRef.current) {
  ...
  const handleRescueVanTracking = async () => {
    ...
    const userIcon = createNeonIcon('#FF3B30', '⚠️');
    strandedMarkerRef.current = window.L.marker(userCenter, { icon: userIcon }).addTo(map);

    const vanIcon = createNeonIcon('#00E576', '🚚');
    rescuerMarkerRef.current = window.L.marker(vanStart, { icon: vanIcon }).addTo(map);

    try {
      const routeUrl = `https://router.project-osrm.org/route/v1/driving/${vanStart[1]},${vanStart[0]};${userCenter[1]},${userCenter[0]}?overview=full&geometries=geojson`;
      const res = await axios.get(routeUrl);
      ...
      if (res.data.routes && res.data.routes[0]) {
        ...
        trackingIntervalRef.current = setInterval(() => {
          ...
          if (progress >= 100) {
            ...
            rescuerMarkerRef.current.bindPopup('<b>Assistance Arrived!</b>').openPopup();
            return;
          }
      ...
      trackingIntervalRef.current = setInterval(() => {
        ...
        if (onTrackingProgress) onTrackingProgress(progress);
        const nextLat = vanStart[0] + (userCenter[0] - vanStart[0]) * (progress / 100);
        const nextLng = vanStart[1] + (userCenter[1] - vanStart[1]) * (progress / 100);
    ...
    <div className="relative w-full h-[480px] bg-[#0c0d12] rounded-xl overflow-hidden border border-cyber-gray-800 shadow-cyber-glow">
      ...
      {loadingRoute && (
        <div className="absolute inset-0 bg-[#0c0d12]/80 z-[1000] flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-xs text-cyber-green font-mono uppercase tracking-widest animate-pulse">Decoding real OSRM road coordinates...</p>
        </div>
      )}
      ...
      <div className="absolute bottom-4 right-4 bg-[#121212]/90 border border-cyber-gray-800 p-2 rounded-lg text-xs text-gray-400 flex items-center space-x-2 backdrop-blur-md z-[1000]">
        <Compass className="w-4 h-4 text-cyber-green animate-spin" style={{ animationDuration: '10s' }} />
        <span className="font-mono tracking-widest text-[9px] uppercase">OSM LIVE GRID • CartoDB DARK</span>
      </div>

      {mode === 'route' && !loadingRoute && routePointsRef.current.length > 0 && (
        <div className="absolute top-4 left-4 bg-[#121212]/95 border border-cyber-gray-800 p-3.5 rounded-lg flex items-center space-x-3.5 backdrop-blur-md shadow-lg z-[1000]">
          ...
          >
            {navPlaying ? <Pause className="w-4 h-4 fill-black" /> : <Play className="w-4 h-4 fill-black" />}
          </button>
          ...
          <div className="flex flex-col text-xs pr-2">
            <span className="font-bold text-white uppercase tracking-wider flex items-center">
              ...
              {navPlaying ? 'Simulating Drive...' : carProgress >= 100 ? 'Trip Completed' : 'Navigation Paused'}
            </span>
      ...
      {selectedPin && (
        <div className="absolute bottom-4 left-4 right-4 md:right-auto md:w-80 bg-[#121212]/95 border border-cyber-green/45 p-4 rounded-xl backdrop-blur-md shadow-2xl animate-fade-in z-[1000]">
          <div className="flex justify-between items-start mb-2">
            <h4 className="font-bold text-white text-sm">{selectedPin.name || selectedPin.providerName}</h4>
            <button 
              type="button"
          ...
          <div className="text-xs space-y-2 text-gray-400">
            <p className="flex items-center">
              ...
              <span>{selectedPin.type || 'Charging Port'} • {selectedPin.power || 60} kW</span>
            </p>
            <p className="flex items-center">
              <span className={`w-2 h-2 rounded-full mr-2 ${selectedPin.status === 'Busy' ? 'bg-red-500' : 'bg-cyber-green'}`} />
              <span className="capitalize">{selectedPin.status || 'Active Operations'}</span>
            </p>
            {selectedPin.rating && (
              <p className="text-cyber-green font-semibold">★ {selectedPin.rating} (Highly Rated)</p>
            )}
          </div>
        </div>
      )}
          ...
          {rescueProgress < 100 ? (
            <>
              ...
              <div className="flex flex-col">
                <h4 className="font-bold text-white text-sm">Rescuer is En Route</h4>
                <p className="text-xs text-gray-400 mt-0.5">Assisting Unit: {activeProvider.providerName}</p>
                <div className="flex space-x-3 text-xs text-cyber-green font-semibold mt-2">
                  <span>ETA: {Math.max(1, Math.round(15 - (15 * rescueProgress / 100)))} mins</span>
                  <span>Dist: {(4.2 - (4.2 * rescueProgress / 100)).toFixed(1)} km</span>
                </div>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="flex flex-col">
                <h4 className="font-bold text-cyber-green text-sm">Rescue Unit Arrived!</h4>
                <p className="text-xs text-gray-400 mt-0.5">Mobile charging van has reached your location. Starting emergency charge.</p>
              </div>
            </>
          )}
        </div>
