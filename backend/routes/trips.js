...
const ROUTE_PRESETS = [
  {
    ...
    stops: [
      ...
      },
      {
        ...
        address:
          "NH44 Bypass Road, Near Evolve Tech Center, Anantapur, Andhra Pradesh - 515001",
    ...
    stops: [
      {
        ...
        address:
          "NH65 Bypass Road, Near Suryapet Toll Gates, Suryapet, Telangana - 508213",
...
router.post("/plan", async (req, res) => {
  ...
  if (!source || !destination || startBattery === undefined) {
    return res
      ...
      .json({
        success: false,
        message:
          "Please provide source, destination, and starting battery status",
      });
  }
  ...
  try {
      ...
      try {
        ...
        const decoded = jwt.verify(
          ...
          process.env.JWT_SECRET ||
            "onwheel_ev_jwt_secret_cyber_security_key_987654321",
        );
        user = await User.findById(decoded.id);
      } catch (err) {
        console.warn(
          "Optional JWT verification failed in plan route:",
          err.message,
        );
      }
    }
    ...
    if (user) {
      ...
      if (currentCredits <= 0) {
        return res.status(400).json({
          success: false,
          message:
            "You have run out of premium API intelligence tokens. Please refill them from your dashboard Wallet to continue planning trips.",
        });
      }
    }
    ...
    // Disabled hardcoded presets to enforce 100% Ola Maps dynamic routing for all paths!
    let matchedRoute = null;

    // Default to dynamic route generation
    let distance = 250; // default km fallback if coordinates are missing
    let duration = "4 h 30 m";
    let suggestedStops = [];
    ...
    if (!matchedRoute && sourceCoords && destCoords) {
      ...
      const a =
        ...
        Math.cos((sourceCoords.lat * Math.PI) / 180) *
    ...
    if (matchedRoute) {
      ...
      const arrivalBattery = Math.max(
        3,
        Math.round(tempBattery - finalSegBatteryDrop),
      );
...
router.get("/history", protect, async (req, res) => {
  try {
    const trips = await TripHistory.find({ userId: req.user._id }).sort({
      createdAt: -1,
    });
  ...
  try {
    ...
    if (!trip) {
      return res
        ...
        .json({
          success: false,
          message: "Trip history entry not found or unauthorized",
        });
    }
    await trip.deleteOne();
    res.json({
      success: true,
      message: "Trip history entry removed successfully",
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
